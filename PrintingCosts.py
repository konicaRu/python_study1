def PrintingCosts(Line):
    slash = '/'
    slash_b = slash.replace('/', '\\')
    printing_cost = {' ': 0, '!': 9, '"': 6, '#': 24, '$': 29, '%': 22, '&': 24, "'": 3, '(': 12, ')': 12, '*': 17,
                     '+': 13, ',': 7, '-': 7, '.': 4, '/': 10, '0': 22, '1': 19,
    ...
    for i in Line:
        count = 0
        for j in printing_cost:
            if i == j:
                summ_cost += printing_cost[j]
            if i != j:
                count += 1
            if count == len(printing_cost):
                summ_cost += 23
